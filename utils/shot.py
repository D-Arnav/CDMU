...
def train_shot(classifier, config):
    ...
    for k, v in B.named_parameters():
        ...
        param_group += [{'params': v, 'lr': 3e-2 * 0.1}]

    for k, v in F.named_parameters():
        v.requires_grad = True
        param_group += [{'params': v, 'lr': 3e-2 * 1.0}]

    for k, v in R.named_parameters():
        v.requires_grad = True
        param_group += [{'params': v, 'lr': 3e-2 * 1.0}]
    
    param_group_mu = deepcopy(param_group)


    optimizer = SGD(param_group)
    lr_scheduler = LambdaLR(optimizer, lambda x: 3e-2 * (1. + 1e-3 * float(x)) ** -0.9)
...
def train_target_rot(classifier, config):
    ...
    for epoch in range(2):
        ...
        for i in tqdm(range(1000), desc="Training Rotation Classifier"):
        ...
        if (not config['fast_train']) or (epoch == 2 - 1):
...
def train_forget_shot(classifier, config):
    ...
    for k, v in H.named_parameters():
        v.requires_grad = True
        param_group_mu += [{'params': v, 'lr': 3e-1 * 1.0}]

    optimizer = SGD(param_group)
    mu_optimizer = SGD(param_group_mu)
    ...
    for epoch in range(config['epochs']):
        ...
        for i in tqdm(range(config['iter_per_epoch']), desc=f'SHOT Epoch {epoch+1}'):            
            ...
            adversarial.update(deepcopy(classifier).to(config['device'])) # Weird interaction if not copying over classifier
